...
ddef predict_price(input_df):
    st.write("Порядок feature_columns (первые 5):", feature_columns[:5])
    st.write("Порядок num_cols:", num_cols)
    st.write("Порядок cat_cols:", cat_cols)
    ...
    st.write("Порядок X_final.columns (первые 5):", X_final.columns[:5])
